    ...
    withdrawal: (state, { payload }) => {
      ...
      state.history.push();
        ...
        deposit: (state, action) => {
          ...
          state.history.push({ type: 'deposit', amount, date: new Date().toISOString() });
        },
    
        //  Add the transfer reducer
        transfer: (state, action) => {
          const amount = action.payload;
          if (state.balance >= amount) {
            state.balance -= amount;
            state.history.push({ type: 'transfer', amount, date: new Date().toISOString() });
          }
        }
      }
    );
